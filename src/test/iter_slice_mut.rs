...
fn forward() {
    ...
    assert_eq!((&10, &mut [0, 1, 2][..]), iter.next().unwrap());
    assert_eq!((&20, &mut [0, 1][..]), iter.next().unwrap());
...
fn reverse() {
    ...
    assert_eq!((&20, &mut [0, 1][..]), iter.next_back().unwrap());
    assert_eq!((&10, &mut [0, 1, 2][..]), iter.next_back().unwrap());
    assert_de_iter_empty(iter);
}
